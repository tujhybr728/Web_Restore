...
namespace WebStore.DAL
{
    public class DbInitializer
    {
        ...
        private static void EnsureRoleToUser(IServiceProvider services, string userName, string roleName, string pass)
        {
            ...
            if (userManager.CreateAsync(adminUser, pass).Result.Succeeded) // добавляем пользователя в БД
                userManager.AddToRoleAsync(adminUser, roleName).Wait(); // даем ему роль админа
        }
